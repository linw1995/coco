...
impl std::fmt::Debug for ResolvedCompletionRequest {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            ...
            .field("trace_node_appender", &self.trace_node_appender)
            .field(
                "trace_node_store",
                &self.trace_node_store.as_ref().map(|_| "TraceNodeStore(..)"),
            )
            .finish()
    }
}
...
type TraceNodeStore = dyn NodeStore + Send + Sync;
...
fn resolve_base_url(base_url: Option<&str>) -> std::result::Result<Option<String>, BackendError> {
    match base_url {
        Some(base_url) => resolve_base_url_value(base_url).map(Some),
        None => Ok(None),
    }
}
...
mod tests {
    ...
    impl CompletionBackend for StreamingBackend {
        async fn complete(
            ...
            let trace_node_store = request.trace_node_store.as_deref();
    ...
    #[tokio::test]
    async fn rig_backend_reuses_chatgpt_oauth_client_across_oauth_profiles() {
        with_process_env_async(
            ...
            || async {
                let backend = RigBackend::default();
                let first_config = ChatGptAuthCheckConfig {
                    profile: "chatgpt-main".to_owned(),
                    secrets: BTreeMap::new(),
                    base_url: Some("${COCO_CHATGPT_BASE_URL}".to_owned()),
                };
                let second_config = ChatGptAuthCheckConfig {
                    profile: "chatgpt-secondary".to_owned(),
                    secrets: BTreeMap::new(),
                    base_url: Some("${COCO_CHATGPT_BASE_URL}".to_owned()),
                };

                let _first_client = backend
                    .chatgpt_client(&first_config.secrets, first_config.base_url.as_deref())
                    .expect("auth check client should build");
                let _second_client = backend
                    .chatgpt_client(&second_config.secrets, second_config.base_url.as_deref())
    ...
    #[test]
    fn resolve_base_url_accepts_literal_profile_base_url() {
        assert_eq!(
            resolve_base_url(Some("https://profile.example.test"))
                .unwrap()
                .as_deref(),
    ...
    async fn resolve_base_url_reads_profile_env_reference() {
        with_process_env_async(
            ...
            || async {
                assert_eq!(
                    resolve_base_url(Some("${COCO_WORK_ANTHROPIC_BASE_URL}"))
                        .unwrap()
                        .as_deref(),
    ...
    async fn resolve_base_url_reports_missing_profile_env_reference() {
        with_process_env_async(&[("COCO_WORK_ANTHROPIC_BASE_URL", None)], || async {
            let error = resolve_base_url(Some("${COCO_WORK_ANTHROPIC_BASE_URL}")).unwrap_err();

            assert_eq!(
                error.to_string(),
                "missing base URL in environment variable COCO_WORK_ANTHROPIC_BASE_URL"
            );
        })
        .await;
    }
