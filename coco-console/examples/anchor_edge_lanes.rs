...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let store = SqliteStore::open(&demo_root).await?;
    let root = store.root_id();

    let alpha_0 = append_anchor(&store, &root, "Alpha / 0", Vec::new()).await?;
    let beta_0 = append_anchor(&store, &root, "Beta / 0", Vec::new()).await?;
    let gamma_0 = append_anchor(&store, &root, "Gamma / 0", Vec::new()).await?;
    let delta_0 = append_anchor(&store, &root, "Delta / 0", Vec::new()).await?;
    let epsilon_0 = append_anchor(&store, &root, "Epsilon / 0", Vec::new()).await?;

    let epsilon_1 = append_anchor(&store, &epsilon_0, "Epsilon / 1", Vec::new()).await?;
    let delta_1 = append_anchor(&store, &delta_0, "Delta / 1", Vec::new()).await?;
    let gamma_1 = append_anchor(&store, &gamma_0, "Gamma / 1", Vec::new()).await?;
    let beta_1 = append_anchor(&store, &beta_0, "Beta / 1", Vec::new()).await?;
    let alpha_1 = append_anchor(&store, &alpha_0, "Alpha / 1", Vec::new()).await?;

    let alpha_2 = append_anchor(
        &store,
        &alpha_1,
        "Alpha / 2",
    ...
    .await?;
    let beta_2 = append_anchor(&store, &beta_1, "Beta / 2", Vec::new()).await?;

    let beta_3 = append_anchor(
        &store,
        &beta_2,
        "Beta / 3",
