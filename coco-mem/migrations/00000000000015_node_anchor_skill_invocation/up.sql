CREATE TEMP TABLE node_anchor_skill_invocation_migration_guard (
    complete INTEGER NOT NULL CHECK (complete = 1)
);

INSERT INTO node_anchor_skill_invocation_migration_guard (complete)
SELECT CASE
    WHEN NOT EXISTS (
        SELECT 1
        FROM node_anchors
        WHERE kind = 'skill_invocation'
          AND (
              ...
              OR json_type(kind_json, '$.Anchor.payload.SkillInvocation') <> 'object'
              OR json_type(kind_json, '$.Anchor.payload.SkillInvocation.mode') <> 'object'
          )
    )
    AND NOT EXISTS (
        SELECT 1
